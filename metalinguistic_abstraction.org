...
We can regard the evaluator as a very special machine that takes as input a description of a machine, and in response, configures itself to emulate the machine described. 
...
Then it will evaluate run-forever, and it wouldn't have halted.

Suppose it doesn't halt. Then 'halted will never be evaluated and it won't halt. 

Thus both efforts violate the intended behavior of halts, because it can never actually show the program halting. 

** Internal Definitions 

Our environment model of evaluation and our metacircular evaluator execute definitions in sequence, extending the environment frame one definition at a time. If we consider the internal definitions used to implement block structure, we will find that name-by-name extension of the environment may not be the best way to define local variables. 

Consider a procedure with internal definitions like the following:
...
Here odd? and even? are mutually recursive procedures. The only satisfactory interpretation of the two defines is to regard them as if the names even? and odd? were being added to the environment simultaneously. 

Since the definitions of the internal procedures come first, no calls to these procedures will be evaluated until all of them have been defined. Our sequential evaluation mechanism will give the same result as a mechanism that directly implements our simulataneous definition for any procedure in which the internal definitions come first in a body and evaluation of the value expressions for the defined variables doesn't actually use any of the defined variables.  

There is a way to treat definitions such that internally defined names have truly simultaneous scope -- create all local variables that will be in the current environment before evaluating any of the value expressions. 

We can do this with a syntax transformation on lambda expressions -- before we evaluate the body of a lambda expression, we scan out and eliminate all the internal definitions in the body. The internally defined variables will be created with a let and then set to their values by assignment. 
...
1. Change lookup-variable-value to signal an error if the value it finds is the symbol *unassigned*
...
#+RESULTS:
#[compound-procedure 151 lookup-variable-value]

2. Write a procedure scale-out-defines that takes a procedure body and returns an equivalent one that has no internal definitions, by making the transformation above 
...
(define (scan-out-defines body)
  ; build out lists of definitions and expressions
  ...
  (let ((pair (collect body '() '())))
    ; take lists and apply the assignment functions to them
    (make-let (map (lambda (def) (list (definition-variable def) '*unassigned*)) defs)
              (append (map (lambda (def) (make-assignment (definition-variable def)
                                                     (definition-value def))) defs) exps))))
#+END_SRC

#+RESULTS:
#[constant 147 #x2]

3. Install scan-out-defines in the interpreter, either in make-procedure or in procedure-body. What place is better? Why? 

We would want to place scan-out-defines in make-procedure so that we aren't scanning every time the method recursively looked at the procedure-body

*** Exercise 4.20 

Because internal definitions look sequential, but are actually simultaneous, some use letrec instead. letrec binds variables simultaneously and lets them have the same scope as each other. 
...
letrec expressions have the form 

(letrec ((v1 ex1)...(vn exn)) body)

and are a variation on let in which the expressions exk that provide the initial values for the variables vk are evaluated in an environment that includes all the letrec bindings. This permits recursion in the bindings.

1. Implement letrec as a derived expression, by transforming letrec into a let expression as shown above. 

letrec -> let -> assign values with set! 
...
(define (letrec->let exp)
  (list 'let (declare-variables exp)
        (make-begin (append (set-variables exp)
                            (letrec-body exp)))))
#+END_SRC
...
2. Louis Reasoner says we can just use let instead of defines inside a procedure. 

*** Exercise 4.21 

Louis's intuition was correct -- we can specify recursive procedures without using letrec. 
...
1. Check that this really computes factorials, and devise an analogous expression for computing fibonacci numbers 
...
Bonus: Implement the Y Combinator 
...
Fill in the missing expressions to complete an alternative definition of f, which uses neither internal definitions not letrec: 
...
The evaluator implemented above is simple, but inefficient, because the syntactic analysis of expressions is interleaved with their execution. Thus if a program is executed many times, its syntax is analyzed many times. The analysis is expensive, and performing it repeatedly is wasteful. 

We can transform the evaluator to be more efficient by arranging things so that syntactic analysis is performed only once. 

We split eval into analyze and an execution procedure. The analyze procedure takes the expression, performs the syntactic analysis, and returns a new procedure -- the execution procedure. This procedure encapsulates the work to be done in executing the analyzed expression. It takes an environment as its argument and completes the evaluation. As a result, analyze is only called once and the execution procedure may be called many times. 
